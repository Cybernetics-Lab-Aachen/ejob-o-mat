...
func ExecuteAnswers(answers Scheme.Answers) (result Scheme.ProductGroups) {
	...
	for n, productGroup := range data.ProductsCollection.Products {
		...
		/*  2 */ groups[n].Points = groups[n].Points + (kindConditionalPossibility(answers.A2Data, productGroup.SharedProperties.Tutoring) * int(answers.A2Weight))
		...
		/* 12 */ groups[n].Points = groups[n].Points + (kindConditionalPresence(answers.A12Data, productGroup.SharedProperties.Assessments) * int(answers.A12Weight))
